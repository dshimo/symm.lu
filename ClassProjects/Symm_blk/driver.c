...
int main(int argc, char *argv[])
{
  ...
  for ( n=nfirst; n<= nlast; n+=ninc ){
    ...
    /* Time reference implementation (from libflame) */
      ...
      /* Compute Cref = A B + Cref or Cref = B A + Cref,
	 where A is symmetric stored in the
	 indicated triangular part of array A, by calling FLA_Symm.  The
	 result ends up in Cef, which we will consider to be the correct
	 result. */
      FLA_Symm( SIDE, UPLO, FLA_ONE, Aobj, Bobj, FLA_ONE, Cref );
